				...
				if (newQueue.repeatMode == 0) {
					...
					message.reply({
						embeds: [new MessageEmbed()
						  .setColor(ee.color)
						  .setTimestamp()
						  .setDescription(`<:loop_button:882513401937551370> **Enabled the __Song__-Loop** ||(Disabled the **Queue-Loop**)||`)
						  .setFooter(` Action by: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true}))]
						})
				} else {
					message.reply({
						embeds: [new MessageEmbed()
						  .setColor(ee.color)
						  .setTimestamp()
						  .setDescription(`<:loop_button:882513401937551370> **Enabled the __Queue__-Loop!** ||(Disabled the **Song-Loop**)||`)
						  .setFooter(` Action by: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true}))]
						})
				}
			} catch (e) {
