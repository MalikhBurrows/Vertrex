	...
	run: async (client, message, args) => {
		try {
			...
			let newmsg = await message.reply({
				content: `<:search_button:882897892464005200> Searching... \`\`\`${Text}\`\`\``,
			}).catch(e => {
				console.log(e)
			})
			try {
				...
				//Edit the reply
				newmsg.edit({
					content: `${queue?.songs?.length > 0 ? "<:add1:887696095742922762> Added to the Top of the Queue" : "<:playing:887696061001498665> Now Playing"}: \`\`\`css\n${Text}\n\`\`\``,
				}).catch(e => {
					console.log(e)
				})
			} catch (e) {
