...
function onCoolDown(message, command) {
  ...
  function GetUser(message, arg){
    var errormessage = "<:reject_button:882546072440938496> I failed finding that User...";
  ...
  function GetRole(message, arg){
    var errormessage = "<:reject_button:882546072440938496> I failed finding that Role...";
    return new Promise(async (resolve, reject) => {
      var args = arg, client = message.client;
      if(!client || !message) return reject("CLIENT IS NOT DEFINED")
      if(!args || args == null || args == undefined) args = message.content.trim().split(/ +/).slice(1);
  ...
  function change_status(client) {
    try {
      client.user.setActivity("+help | V.09", {
    type:"STREAMING",
    url: "https://www.twitch.tv/Vextrex"
      });
    } catch (e) {
      console.log(String(e.stack).bgRed)
    }
